 ...
 */
function getString(section: IniSection | undefined, key: string): string | undefined {
 ...
 */
function getNumber(section: IniSection | undefined, key: string, defaultValue = 0): number {
 ...
 */
function getBoolean(section: IniSection | undefined, key: string, defaultValue = false): boolean {
  if (!section) return defaultValue
  const value = section[key.toLowerCase()]
  if (value === undefined) return defaultValue
...
function normalizeState(state: string | undefined): State | undefined {
  ...
  return isValidState(upper) ? upper as State : undefined
}
...
function parse1040Section(ini: ParsedIni): {
  ...
  const filingStatus = parseFilingStatus(getString(section, 'filing_status') ?? getString(section, 'status'))

  // Parse primary filer
  const address: Address = {
    address: getString(section, 'address') ?? getString(section, 'street') ?? '',
  ...
  const primaryPerson: PrimaryPerson = {
    firstName: getString(section, 'first_name') ?? getString(section, 'tp_first_name') ?? '',
    lastName: getString(section, 'last_name') ?? getString(section, 'tp_last_name') ?? '',
    ssid: (getString(section, 'ssn') ?? getString(section, 'tp_ssn') ?? '').replace(/-/g, ''),
    role: PersonRole.PRIMARY,
    isBlind: getBoolean(section, 'blind') || getBoolean(section, 'tp_blind'),
    dateOfBirth: getDate(section, 'dob') || getDate(section, 'date_of_birth') || getDate(section, 'tp_dob'),
    address,
  ...
  if (getString(section, 'spouse_first_name') || getString(section, 'sp_first_name')) {
    spouse = {
      firstName: getString(section, 'spouse_first_name') ?? getString(section, 'sp_first_name') ?? '',
      lastName: getString(section, 'spouse_last_name') ?? getString(section, 'sp_last_name') ?? '',
      ssid: (getString(section, 'spouse_ssn') ?? getString(section, 'sp_ssn') ?? '').replace(/-/g, ''),
      role: PersonRole.SPOUSE,
      isBlind: getBoolean(section, 'spouse_blind') || getBoolean(section, 'sp_blind'),
  ...
  const stateResidencies: StateResidency[] = residencyState ? [{ state: residencyState }] : []
...
function parseDependents(ini: ParsedIni): Dependent[] {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    dependents.push({
      firstName: getString(section, 'first_name') ?? getString(section, 'name')?.split(' ')[0] ?? '',
      lastName: getString(section, 'last_name') ?? getString(section, 'name')?.split(' ').slice(1).join(' ') ?? '',
...
function parseW2s(ini: ParsedIni): IncomeW2[] {
  ...
  for (let i = 0; i < 20; i++) {
    const section = ini[`w-2:${i}`] || ini[`w2:${i}`] || ini[`w-2-${i}`] || ini[`w2-${i}`]
    if (!section) continue

    const employer: Employer | undefined = getString(section, 'employer') ? {
      EIN: getString(section, 'ein') ?? getString(section, 'employer_ein'),
      employerName: getString(section, 'employer') ?? getString(section, 'employer_name'),
      address: {
    ...
    const isSpouse = getBoolean(section, 'spouse') || getString(section, 'who')?.toLowerCase() === 'spouse'

    w2s.push({
      occupation: getString(section, 'occupation') ?? 'Employee',
      income: getNumber(section, 'wages') || getNumber(section, 'box1') || getNumber(section, 'income'),
      medicareIncome: getNumber(section, 'medicare_wages') || getNumber(section, 'box5') || getNumber(section, 'wages'),
      fedWithholding: getNumber(section, 'federal_withholding') || getNumber(section, 'box2') || getNumber(section, 'fed_withheld'),
      ssWages: getNumber(section, 'ss_wages') || getNumber(section, 'box3') || getNumber(section, 'social_security_wages'),
      ssWithholding: getNumber(section, 'ss_withholding') || getNumber(section, 'box4') || getNumber(section, 'social_security_withheld'),
      medicareWithholding: getNumber(section, 'medicare_withholding') || getNumber(section, 'box6') || getNumber(section, 'medicare_withheld'),
      employer,
      personRole: isSpouse ? PersonRole.SPOUSE : PersonRole.PRIMARY,
      state: normalizeState(getString(section, 'state') ?? getString(section, 'box15')),
      stateWages: getNumber(section, 'state_wages') || getNumber(section, 'box16') || undefined,
      stateWithholding: getNumber(section, 'state_withholding') || getNumber(section, 'box17') || undefined
    })
  }
...
function parse1099Ints(ini: ParsedIni): Supported1099[] {
  ...
  for (let i = 0; i < 20; i++) {
    const section = ini[`1099-int:${i}`] || ini[`1099int:${i}`] || ini[`1099-int-${i}`]
    if (!section) continue

    const isSpouse = getBoolean(section, 'spouse') || getString(section, 'who')?.toLowerCase() === 'spouse'

    forms.push({
      payer: getString(section, 'payer') ?? getString(section, 'bank') ?? 'Unknown Bank',
      type: Income1099Type.INT,
      form: {
        income: getNumber(section, 'interest') || getNumber(section, 'box1') || getNumber(section, 'amount')
      } as F1099IntData,
      personRole: isSpouse ? PersonRole.SPOUSE : PersonRole.PRIMARY
    })
  }
...
function parse1099Divs(ini: ParsedIni): Supported1099[] {
  ...
  for (let i = 0; i < 20; i++) {
    const section = ini[`1099-div:${i}`] || ini[`1099div:${i}`] || ini[`1099-div-${i}`]
    if (!section) continue

    const isSpouse = getBoolean(section, 'spouse') || getString(section, 'who')?.toLowerCase() === 'spouse'

    forms.push({
      payer: getString(section, 'payer') ?? getString(section, 'broker') ?? 'Unknown Broker',
      type: Income1099Type.DIV,
      form: {
        dividends: getNumber(section, 'ordinary_dividends') || getNumber(section, 'box1a') || getNumber(section, 'dividends'),
        qualifiedDividends: getNumber(section, 'qualified_dividends') || getNumber(section, 'box1b'),
        totalCapitalGainsDistributions: getNumber(section, 'capital_gains') || getNumber(section, 'box2a')
      } as F1099DivData,
      personRole: isSpouse ? PersonRole.SPOUSE : PersonRole.PRIMARY
    })
  }
...
function parse1099Bs(ini: ParsedIni): Supported1099[] {
  ...
  for (let i = 0; i < 20; i++) {
    const section = ini[`1099-b:${i}`] || ini[`1099b:${i}`] || ini[`1099-b-${i}`]
    if (!section) continue
    ...
    forms.push({
      ...
      form: {
        shortTermProceeds: getNumber(section, 'short_term_proceeds') || getNumber(section, 'st_proceeds'),
        shortTermCostBasis: getNumber(section, 'short_term_basis') || getNumber(section, 'st_basis'),
        longTermProceeds: getNumber(section, 'long_term_proceeds') || getNumber(section, 'lt_proceeds'),
        longTermCostBasis: getNumber(section, 'long_term_basis') || getNumber(section, 'lt_basis')
      } as F1099BData,
      personRole: isSpouse ? PersonRole.SPOUSE : PersonRole.PRIMARY
    })
  }
...
function parseScheduleA(ini: ParsedIni): ItemizedDeductions | undefined {
  ...
  const isItemizing = getBoolean(section, 'itemize') ||
  ...
  return {
    medicalAndDental: getNumber(section, 'medical') || getNumber(section, 'medical_dental'),
    stateAndLocalTaxes: getNumber(section, 'state_local_taxes') || getNumber(section, 'salt') || getNumber(section, 'state_income_tax'),
    isSalesTax: getBoolean(section, 'sales_tax'),
    stateAndLocalRealEstateTaxes: getNumber(section, 'real_estate_taxes') || getNumber(section, 'property_tax'),
    stateAndLocalPropertyTaxes: getNumber(section, 'personal_property_tax'),
    interest8a: getNumber(section, 'mortgage_interest') || getNumber(section, 'home_mortgage_interest'),
    interest8b: getNumber(section, 'mortgage_points') || getNumber(section, 'points_paid'),
    interest8c: 0,
    interest8d: 0,
    investmentInterest: getNumber(section, 'investment_interest'),
    charityCashCheck: getNumber(section, 'charity_cash') || getNumber(section, 'charitable_cash') || getNumber(section, 'charity'),
    charityOther: getNumber(section, 'charity_noncash') || getNumber(section, 'charitable_noncash')
  }
}
...
function parseRefund(ini: ParsedIni): Refund | undefined {
  ...
  const routingNumber = getString(section, 'routing') ?? getString(section, 'routing_number')
  const accountNumber = getString(section, 'account') ?? getString(section, 'account_number')

  if (!routingNumber || !accountNumber) return undefined

  const accountTypeStr = getString(section, 'account_type') ?? getString(section, 'type')
  const accountType = accountTypeStr?.toLowerCase() === 'savings'
    ? AccountType.savings
...
function parseEstimatedPayments(ini: ParsedIni): EstimatedTaxPayments[] {
  const section = ini['estimated'] || ini['estimated_tax'] || ini['estimated-payments']
  if (!section) return []
...
function parseF1098es(ini: ParsedIni): F1098e[] {
  ...
  if (singleSection) {
    const interest = getNumber(singleSection, 'interest') || getNumber(singleSection, 'box1') || getNumber(singleSection, 'amount')
    if (interest > 0) {
      forms.push({
        lender: getString(singleSection, 'lender') ?? getString(singleSection, 'servicer') ?? 'Student Loan Servicer',
        interest
      })
    }
  }
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const interest = getNumber(section, 'interest') || getNumber(section, 'box1') || getNumber(section, 'amount')
    if (interest > 0) {
      forms.push({
        lender: getString(section, 'lender') ?? getString(section, 'servicer') ?? 'Student Loan Servicer',
        interest
      })
    }
  }
...
function parseHSA(ini: ParsedIni): HealthSavingsAccount[] {
  ...
  const contributions = getNumber(section, 'contributions') || getNumber(section, 'amount')
  ...
  return [{
    ...
    qualifiedDistributions: getNumber(section, 'qualified_distributions') || getNumber(section, 'distributions')
  }]
}
...
export function parseHabutax(iniContent: string): Information {
  ...
  const { filingStatus, primaryPerson, spouse, stateResidencies } = parse1040Section(ini)
  ...
  return {
    ...
    taxPayer,
    questions: {
      CRYPTO: getBoolean(ini['1040'], 'crypto') || getBoolean(ini['1040'], 'virtual_currency'),
...
export function validateHabutax(iniContent: string): {
  ...
  if (!section1040) {
    ...
    if (!getString(section1040, 'first_name') && !getString(section1040, 'tp_first_name')) {
      errors.push('Missing taxpayer first name')
    }
    if (!getString(section1040, 'last_name') && !getString(section1040, 'tp_last_name')) {
    ...
    if (!getString(section1040, 'filing_status') && !getString(section1040, 'status')) {
