...
function getYearValue(
  ...
  const years = Object.keys(param.value).map(Number).sort((a, b) => a - b)
...
function getYearArrayValue(
  ...
  const yearValue = param.value[String(applicableYear)]
  if (yearValue === undefined) return defaultValue
  if (Array.isArray(yearValue)) return yearValue as number[]
  return [yearValue as number]
}
...
function extractBrackets(
  ...
  const rateParams = ['II_rt1', 'II_rt2', 'II_rt3', 'II_rt4', 'II_rt5', 'II_rt6', 'II_rt7']
  const bracketParams = ['II_brk1', 'II_brk2', 'II_brk3', 'II_brk4', 'II_brk5', 'II_brk6']

  const rates = rateParams.map(param =>
    getYearValue(policy[param], year, filingStatus, 0)
  ).filter(r => r > 0)

  const brackets = bracketParams.map(param =>
    getYearValue(policy[param], year, filingStatus, Infinity)
  ).filter(b => b < Infinity)
...
function extractDeductions(
  ...
  return {
    ...
    additionalStandardDeduction: getYearValue(policy['STD_Aged'], year, filingStatus, 0),
...
function extractEITC(
  ...
  return {
    ...
    phaseInRate: getYearArrayValue(policy['EITC_prt'], year, [0.0765, 0.34, 0.40, 0.45]),
    phaseOutStart: getYearArrayValue(policy['EITC_ps'], year, [9800, 21900, 21900, 21900]),
    phaseOutStartMFJ: getYearArrayValue(policy['EITC_ps'], year, [16500, 28500, 28500, 28500]),
    phaseOutRate: getYearArrayValue(policy['EITC_prt'], year, [0.0765, 0.1598, 0.2106, 0.2106]),
    maxInvestmentIncome: getYearValue(policy['EITC_InvestIncome_c'], year, filingStatus, 11000)
  }
}
...
function extractCTC(
  ...
  return {
    ...
    maxCreditOtherDependents: getYearValue(policy['ODC_c'], year, filingStatus, 500),
    phaseOutStart: getYearValue(policy['CTC_ps'], year, FilingStatus.S, 200000),
    phaseOutStartMFJ: getYearValue(policy['CTC_ps'], year, FilingStatus.MFJ, 400000),
...
function extractCapitalGains(
  ...
  return {
    ...
    rate15Threshold: getYearValue(policy['CG_brk2'], year, filingStatus, 518900),
    rate20Threshold: getYearValue(policy['CG_brk3'], year, filingStatus, Infinity),
...
function extractSocialSecurity(
  ...
  return {
    wageBase: getYearValue(policy['SS_Earnings_c'], year, FilingStatus.S, 168600),
    taxRate: 0.062,  // 6.2%
...
function extractMedicare(
  ...
  return {
    ...
    additionalTaxThreshold: getYearValue(policy['AMEDT_thd'], year, FilingStatus.S, 200000),
    additionalTaxThresholdMFJ: getYearValue(policy['AMEDT_thd'], year, FilingStatus.MFJ, 250000)
  }
}
...
export function parseTaxCalculatorPolicy(
  ...
  const policy: TaxCalculatorPolicy = typeof policyJson === 'string'
  ...
  return {
    ...
    saltCap: getYearValue(policy['ID_StateLocalTax_hc'], year, filingStatus, 10000),
    qbiDeductionRate: getYearValue(policy['PT_qbid_rt'], year, filingStatus, 0.20),
    studentLoanInterestMax: getYearValue(policy['ID_Charity_crt_all'], year, filingStatus, 2500),
...
export function generateTypeScriptConstants(
  ...
  const params: Record<FilingStatus, TaxParameters> = {
    ...
    [FilingStatus.MFJ]: parseTaxCalculatorPolicy(policy, year, FilingStatus.MFJ),
    [FilingStatus.MFS]: parseTaxCalculatorPolicy(policy, year, FilingStatus.MFS),
    [FilingStatus.HOH]: parseTaxCalculatorPolicy(policy, year, FilingStatus.HOH),
...
export const taxBrackets = {
  ...
  status: {
    [FilingStatus.S]: { brackets: [${params[FilingStatus.S].brackets.brackets.join(', ')}] },
    [FilingStatus.MFJ]: { brackets: [${params[FilingStatus.MFJ].brackets.brackets.join(', ')}] },
    [FilingStatus.MFS]: { brackets: [${params[FilingStatus.MFS].brackets.brackets.join(', ')}] },
    [FilingStatus.HOH]: { brackets: [${params[FilingStatus.HOH].brackets.brackets.join(', ')}] },
    [FilingStatus.W]: { brackets: [${params[FilingStatus.W].brackets.brackets.join(', ')}] }
  }
}
 ...
 */
export function getAvailableYears(policyJson: string | TaxCalculatorPolicy): number[] {
  const policy: TaxCalculatorPolicy = typeof policyJson === 'string'
    ? JSON.parse(policyJson)
    : policyJson
...
export function compareYears(
  ...
  year2: number,
  filingStatus: FilingStatus
): Record<string, { year1: number; year2: number; change: number; percentChange: string }> {
  ...
  const differences: Record<string, { year1: number; year2: number; change: number; percentChange: string }> = {}

  // Compare key numeric values
  const comparisons: Array<[string, number, number]> = [
    ['standardDeduction', params1.deductions.standardDeduction, params2.deductions.standardDeduction],
    ['ssWageBase', params1.socialSecurity.wageBase, params2.socialSecurity.wageBase],
    ...
    ['capitalGains0%Threshold', params1.capitalGains.rate0Threshold, params2.capitalGains.rate0Threshold]
  ]

  for (const [name, val1, val2] of comparisons) {
    if (val1 !== val2) {
      const change = val2 - val1
      const percentChange = val1 !== 0 ? ((change / val1) * 100).toFixed(2) + '%' : 'N/A'
...
// Export types
export type { TaxCalculatorPolicy, PolicyParameter, YearIndexedValue, IndexedValue }
