...
export type DisasterType = 'hurricane' | 'wildfire' | 'flood' | 'tornado' | 'earthquake' | 'other'
...
export default class F8915F extends F1040Attachment {
  ...
  disasterDistributions = (): QualifiedDisasterDistribution[] => {
    return (this.f1040.info.disasterDistributions as QualifiedDisasterDistribution[] | undefined) ?? []
  }
  ...
  l2 = (): number => {
    return this.disasterDistributions()
      ...
      .reduce((sum, d) => sum + Math.min(d.distributionAmount, this.maxPerDisaster()), 0)
  }
  ...
  l9 = (): number => {
    return this.disasterDistributions()
      .reduce((sum, d) => sum + d.repaymentAmount, 0)
  }
