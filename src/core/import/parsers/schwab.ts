...
export class SchwabParser extends BaseBrokerageParser {
  ...
  canParse(content: string, headers: string[]): boolean {
    ...
    const hasSchwabColumns =
      ...
      (headerLine.includes('covered or noncovered') &&
  ...
  parse(content: string): ParseResult {
    ...
    for (let i = 0; i < Math.min(rows.length, 15); i++) {
      ...
      if (
        ...
        rowLower.some(
          (c) =>
            c.includes('date') && (c.includes('acquired') || c.includes('sold'))
        )
      ) {
        headerRowIndex = i
        break
    ...
    const columnMap = {
      ...
      description: findColumn(
        actualHeaders,
        SCHWAB_COLUMN_MAPPINGS.description
      ),
      dateAcquired: findColumn(
        actualHeaders,
        SCHWAB_COLUMN_MAPPINGS.dateAcquired
      ),
      ...
      adjustmentCode: findColumn(
        actualHeaders,
        SCHWAB_COLUMN_MAPPINGS.adjustmentCode
      ),
      adjustmentAmount: findColumn(
        actualHeaders,
        SCHWAB_COLUMN_MAPPINGS.adjustmentAmount
      )
    }
      ...
      try {
        ...
        if (dateAcquiredStr.toLowerCase() === 'various') {
          warnings.push(
            `Row ${
              i + 1
            }: "Various" date acquired - using sale date minus 1 year as estimate for ${symbol}`
          )
        ...
        if (columnMap.covered >= 0) {
          ...
          if (
            coveredValue === 'c' ||
            coveredValue === 'f' ||
            coveredValue.includes('noncovered') ||
            coveredValue.includes('non-covered')
          ) {
            isCovered = false
          } else if (
            coveredValue === 'a' ||
            coveredValue === 'd' ||
            coveredValue.includes('covered')
          ) {
            isCovered = true
          }
        }
