  ...
  canParse(content: string, headers: string[]): boolean {
    ...
    const hasRequiredColumns =
      (headerLine.includes('date acquired') ||
        headerLine.includes('acquisition')) &&
  ...
  parse(content: string): ParseResult {
    ...
    if (columnMap.symbol < 0) {
      errors.push({
        row: headerRowIndex,
        column: 'symbol',
        message: 'Could not find Symbol column'
      })
    }
      ...
      try {
        ...
        if (!dateAcquired) {
          errors.push({
            row: i + 1,
            column: 'dateAcquired',
            message: `Invalid date acquired: ${row[columnMap.dateAcquired]}`
          })
          continue
        }
        ...
        if (!dateSold) {
          errors.push({
            row: i + 1,
            column: 'dateSold',
            message: `Invalid date sold: ${row[columnMap.dateSold]}`
          })
          continue
        }
        ...
        const quantity =
          columnMap.quantity >= 0 ? parseFloat(row[columnMap.quantity]) || 1 : 1
        ...
        if (columnMap.covered >= 0) {
          ...
          isCovered =
            coveredValue.includes('covered') && !coveredValue.includes('non')
        }

        const transaction: BrokerageTransaction = {
          symbol: symbol.trim(),
          description:
            columnMap.description >= 0 ? row[columnMap.description] : undefined,
          dateAcquired,
          dateSold,
          proceeds,
          costBasis,
