...
try {
  ...
  if (fs.existsSync(outputPath)) {
    console.warn(
      '[setup] Schema generation failed; continuing with the existing validate-fns.js snapshot.'
    )
    console.warn(
      error instanceof Error ? error.message : 'Unknown schema generation error'
    )
  } else {
    throw error
  }
