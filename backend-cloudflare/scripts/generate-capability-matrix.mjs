...
const sharedEntityEndpoints = [
  'POST /app/v1/filing-sessions',
  'GET /app/v1/filing-sessions/:sessionId',
  'PATCH /app/v1/filing-sessions/:sessionId',
  'GET /app/v1/filing-sessions/:sessionId/entities',
  'PUT /app/v1/filing-sessions/:sessionId/entities/:entityType/:entityId',
  'DELETE /app/v1/filing-sessions/:sessionId/entities/:entityType/:entityId',
  'POST /app/v1/filing-sessions/:sessionId/returns/sync'
]

const rows = [
  {
    family: 'auth_and_sessions',
    entityTypes: ['taxpayer'],
    taxflowRoutes: ['/session-dashboard', '/interview', '/taxpayer-profile'],
    backendEndpoints: [
      'POST /app/v1/auth/dev-login',
      'GET /app/v1/auth/callback',
      'GET /app/v1/auth/me',
      'POST /app/v1/auth/logout',
      ...sharedEntityEndpoints.slice(0, 3)
    ],
    d1Tables: ['users', 'filing_sessions'],
    r2Artifacts: ['filing-sessions/<id>/snapshot.json'],
    orchestration: 'cookie auth + filing session bootstrap',
    atsCoverage: 'runtime e2e',
    directFileCoverage: 'adapter_indirect',
    status: 'implemented_tested',
    notes:
      'Authenticated TaxFlow session bootstrap is working locally with worker runtime coverage.'
  },
  {
    family: 'taxpayer_identity',
    entityTypes: ['taxpayer'],
    ...
    status: 'implemented_tested',
    notes:
      'Core identity, address, filing-status sync, checklist, review, sign, and submit are backend-backed.'
  },
  {
    family: 'spouse_household',
    entityTypes: ['spouse'],
  ...
  },
  {
    ...
    entityTypes: ['dependent'],
    taxflowRoutes: ['/household'],
    backendEndpoints: sharedEntityEndpoints,
  ...
  },
  {
    ...
    entityTypes: ['tax_lot', 'crypto_account', '1099_b'],
  ...
  },
  {
    ...
    entityTypes: ['rental_property'],
    taxflowRoutes: ['/rental'],
    backendEndpoints: sharedEntityEndpoints,
    d1Tables: ['filing_sessions', 'session_entities'],
  ...
  },
  {
    ...
    entityTypes: ['hsa_account', 'ira_distribution'],
  ...
  },
  {
    ...
    entityTypes: ['unemployment_record', 'ssa_record'],
  ...
  },
  {
    ...
    entityTypes: ['local_tax_obligation'],
    taxflowRoutes: ['/state-tax', '/local-tax'],
    backendEndpoints: [
      ...sharedEntityEndpoints,
      'GET /app/v1/filing-sessions/:sessionId/state-transfer',
      'POST /app/v1/filing-sessions/:sessionId/state-transfer/authorize',
  ...
  },
  {
    ...
    entityTypes: [],
    taxflowRoutes: ['/checklist', '/review-confirm'],
    backendEndpoints: [
    ...
    r2Artifacts: ['filing-sessions/<id>/snapshot.json'],
    orchestration: 'backend-derived review findings + checklist aggregation',
    atsCoverage: 'runtime e2e',
    directFileCoverage: 'adapter_indirect',
    status: 'implemented_tested',
    notes:
      'Checklist and review now come from backend routes instead of static frontend fixtures on the core path.'
  },
  {
    family: 'document_intake',
    entityTypes: [],
    taxflowRoutes: ['/documents', '/dump', '/extraction-review'],
    backendEndpoints: [
      'POST /app/v1/filing-sessions/:sessionId/documents',
      'GET /app/v1/filing-sessions/:sessionId/documents/:documentId',
    ...
    status: 'implemented_tested',
    notes:
      'TaxFlow upload flows now persist document metadata to the worker backend, but OCR/classification is still adapter-backed.'
  },
  {
    family: 'efile_submission',
    entityTypes: ['submission', 'bank_account'],
    taxflowRoutes: ['/efile-wizard', '/efile-status-v2', '/confirmation'],
    backendEndpoints: [
      'POST /app/v1/filing-sessions/:sessionId/sign',
      'POST /app/v1/filing-sessions/:sessionId/submit',
      'GET /app/v1/filing-sessions/:sessionId/submission',
      'POST /api/v1/internal/process/:submissionId',
    ...
    status: 'implemented_tested',
    notes:
      'Core TaxFlow e-file submission path is backend-backed and worker-tested end-to-end.'
  },
  {
    family: 'rejection_and_retry',
    entityTypes: ['submission', 'rejection_record'],
    taxflowRoutes: ['/rejection-repair', '/efile-status-v2'],
    backendEndpoints: [
      'GET /app/v1/filing-sessions/:sessionId/submission',
      'POST /api/v1/internal/submissions/:submissionId/retry',
      'GET /api/v1/submissions/:submissionId/ack'
    ],
  ...
  },
  {
    ...
    taxflowRoutes: ['/state-transfer', '/state-tax'],
    backendEndpoints: [
      'GET /app/v1/filing-sessions/:sessionId/state-transfer',
      'POST /app/v1/filing-sessions/:sessionId/state-transfer/authorize',
      'GET /df/file/api/v1/state-api/state-profile',
    ...
    status: 'implemented_tested',
    notes:
      'Authorization and profile retrieval are wired in both app and Direct File compatibility layers.'
  },
  {
...
const csv = [
  ...
  ...rows.map((row) =>
    [
      ...
      row.entityTypes,
      row.taxflowRoutes,
      row.backendEndpoints,
      row.d1Tables,
      row.r2Artifacts,
      row.orchestration,
      row.atsCoverage,
      row.directFileCoverage,
      row.status,
      row.notes,
      today
    ]
...
console.log(
  `Wrote ${rows.length} capability rows to reports/taxflow_backend_capability_matrix.csv`
)
